...
function pageNumCheck(
  ...
  if (pageNum > lastComic) {
    ...
    document.getElementById('display').value = Number(1);
    alert(`latest comics is until ${lastComic}, let me take you there. 😊`);
    displaySet(lastComic, 1);
  } else if (pageNum < 1) {
    document.getElementById('page_Num').value = 1;
    document.getElementById('display').value = Number(1);
    alert(`first page is comic#1, let me take you there. 😊`);
    displaySet(1, 1);
  } else {
    if (lastDisplayPage > lastComic) {
      ...
      if (displayCount == 3) {
        ...
        alert(
          'comics with 3 displays pages can only be set to minimum at comic#2, let me set it up for you. 😊'
        );
        displaySet(2, 3);
      }
      if (displayCount == 5) {
        ...
        alert(
          'comics with 5 displays pages can only be set to minimum at comic#3, let me set it up for you. 😊'
        );
        displaySet(3, 5);
      }
    } else {
...
function setupCycle(lastComic, displayCount, lastPage) {
  ...
  for (let i = 0; i < displayCount; i++) {
    ...
    if (pagetoPost <= lastPage) {
      displayComics(pagetoPost, counter);
    } else {
      newCycle++;
      displayComics(newCycle, counter);
    }
  }
}
...
function getRandomComics() {
  ...
  checkLastPage().then((result) => {
    let randomNum = Math.ceil(Math.random() * result);
...
function displaySet(currentPage, displayCount) {
  ...
  for (let i = firstDisplay; i <= lastDisplay; i++) {
    ...
    displayComics(i, counter);
  }
}

function displayComics(comicNum, counter) {
  displayLoading();
